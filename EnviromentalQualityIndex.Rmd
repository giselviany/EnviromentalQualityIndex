...
```{r}
newdata <- read_excel("gisel/BINUS/Semester 6/Multivariate Statistics/aol/dataset aol.xlsx")
...
```{r}
transform_pt2 <- summary(powerTransform(kedua))

Padi <- kedua[,1]^transform_pt2$result[1]
Jagung <- kedua[,2]^transform_pt2$result[2]
KacangTanah <- kedua[,3]^transform_pt2$result[3]
transformed2 <- tibble(Padi,Jagung,KacangTanah)
...
```{r warning=FALSE}
cor.test(transformed2$Padi, transformed2$Jagung)
cor.test(transformed2$Padi, transformed2$KacangTanah)
cor.test(transformed2$Jagung, transformed2$KacangTanah)
```
```{r}
cor.test(kedua$Padi, kedua$Jagung)
cor.test(kedua$Padi, kedua$KacangTanah)
...
pertama <- newdata[, c("IKA", "IKU", "IKTL")]
kedua <- newdata[, c("Padi", "Jagung", "KacangTanah")]
cca_result_transformed <- cc(transformed, transformed2)
